...
public class OnLine {
	...
	public OnLine(List<SensorNode> list,double length,double r){
		this.nodeList = list;
	...
	public double getMinMaxDistance(){
		...
		if(result1.size()==1){
			//System.out.println("result-1:"+result1.get(0));
...
//				System.out.println("---");
				if(dis3.size()!=0){
					List<Double> result3=binary_search(dis3,this.nodeList);
					   if(result3.size()==1){
						   System.out.println("result3_1:"+feasibleDistance(result3.get(0),this.nodeList));
						   return result3.get(0);
					   }
					   else {
						   System.out.println("result3_2:"+feasibleDistance(result3.get(1),this.nodeList));
						   return result3.get(1);
					   }
				}else{
					System.out.println("result2_2:"+feasibleDistance(result2.get(1),this.nodeList));
			...
			if(set1.size()!=0){
				...
				for(int j=0;j<list.size();j++){
					...
					/* double t1=x+largestMovement-this.radius;
					 ...
					 System.out.println("set3:["+t1+","+t2+"],while r="+rightmost);*/
					if(largestMovement!=-1&&x+largestMovement-this.radius >=rightmost&&x-largestMovement-this.radius <=rightmost){
						if(!criticalSet.contains(this.nodeList.get(j))){
							set2.add(list.get(j));
						   ...
						   if(list.get(j).x_final==list.get(i).x_final+2*this.radius&&list.get(j).x_final<=this.barrier_length-this.radius){
							   ...
							   for(int z=j+1;z<list.size();z++){
								  if(list.get(z).y_final==0){
									  if(list.get(j).x-list.get(j).movement>list.get(z).x-list.get(z).movement){
										  flag=false;
										  break;
									  }
								  }
