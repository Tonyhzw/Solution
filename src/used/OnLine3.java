...
public class OnLine3 {
	...
	public OnLine3(List<SensorNode> list,double length,double r){
		this.nodeList = list;
	...
	public boolean feasibleDistance(double dis,List<SensorNode> list){
		...
		Collections.sort(list,new Comparator<SensorNode>(){
			public int compare(SensorNode o1, SensorNode o2) {
				...
				return new Double(o1.x+o1.movement-o1.r).compareTo(new Double(o2.x+o2.movement-o2.r));
			}			
		});
		...
		while(rightmost<this.barrier_length){
			...
			for(int i=0;i<list.size();i++){
				if(list.get(i).movement!=-1){
					if(list.get(i).x+list.get(i).movement-list.get(i).r<rightmost){
						if(list.get(i).x+list.get(i).movement+list.get(i).r>rightmost){
					...
					if(list.get(i).x+list.get(i).movement-list.get(i).r>=rightmost) break;
				}
			}
